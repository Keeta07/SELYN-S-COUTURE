...
import { api, money, WHATSAPP_NUMBER } from '../lib/api.js';
...
export default function ClientApp() {
  ...
  return (
    <div className="min-h-screen bg-silk text-ink">
      ...
      <main>
        <Hero />
        <FeaturedProducts products={featuredProducts} addToCart={addToCart} whatsappOrder={whatsappOrder} />
        <CategoryShowcase setCategory={setCategory} />
        <CatalogControls category={category} setCategory={setCategory} search={search} setSearch={setSearch} />
      ...
      <Footer />
      {mobileOpen && <MobileMenu setMobileOpen={setMobileOpen} cartCount={cart.length} />}
    </div>
  );
}

function Header({ cartCount, mobileOpen, setMobileOpen }) {
  return (
...
function MobileMenu({ setMobileOpen, cartCount }) {
  return (
...
function FeaturedProducts({ products, addToCart, whatsappOrder }) {
    ...
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        {products.map((product) => (
          <article key={product._id} className="rounded-lg border border-ink/10 bg-white p-4 shadow-sm">
            <img className="aspect-[4/3] w-full rounded-md object-cover" src={product.images?.[0]} alt={product.name} />
            <h3 className="mt-4 text-lg font-bold">{product.name}</h3>
            <p className="mt-1 text-sm text-ink/65">{product.category}</p>
            <div className="mt-4 flex items-center justify-between">
              <strong>{money(product.price)}</strong>
              <div className="flex gap-2">
                <button className="icon-button" onClick={() => addToCart(product)} aria-label={`Add ${product.name}`}>
                  <ShoppingBag size={17} />
                </button>
                <button className="icon-button" onClick={() => whatsappOrder(product)} aria-label={`Order ${product.name}`}>
...
function CategoryShowcase({ setCategory }) {
  const items = [
    ["Men's Wear", 'Tailored native and formal pieces'],
    ["Women's Wear", 'Occasion gowns and everyday elegance'],
    ['Fascinators', 'Event-ready headpieces'],
    ['Bags', 'Fashionable accessories'],
    ['Resin Art', 'Hand-poured art and trays'],
    ['School Uniforms', 'Bulk production for schools']
  ];
  ...
  return (
    <section className="bg-white py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="section-title">Shop by specialty</h2>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map(([title, text]) => (
            <a
              ...
              className="rounded-lg border border-ink/10 bg-silk p-5 transition hover:border-berry"
            >
...
function WhyChooseUs() {
  const items = [
    ['Bespoke fit', 'Pieces are shaped around real measurements, events, and personal style.'],
    ['Multiple specialties', 'Fashion retail, uniforms, art, accessories, and training in one place.'],
    ['Direct follow-up', 'Orders and requests can continue quickly through WhatsApp.']
  ];

  return (
    <section className="bg-ink py-16 text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="section-kicker text-blush">Why choose us</p>
        <h2 className="section-title text-white">Fashion work with detail and accountability</h2>
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {items.map(([title, text]) => (
            <div key={title} className="rounded-lg border border-white/10 bg-white/[0.08] p-6">
              <h3 className="font-bold">{title}</h3>
...
function SpecialtySections() {
  const sections = [
    ['Fashion Training', 'Apply for hands-on sewing, pattern drafting, finishing, and fashion business training.'],
    ['School Uniforms', 'Submit bulk uniform requirements for nursery, primary, and secondary school production.'],
    ['Resin Art', 'Order custom resin trays, decor pieces, and thoughtful handmade gifts.']
  ];
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {sections.map(([title, text]) => (
          <div key={title} className="rounded-lg border border-ink/10 bg-white p-6 shadow-sm">
            ...
            <a href="#requests" className="secondary-button mt-5">Start Request</a>
          </div>
        ))}
      </div>
    </section>
...
function Testimonials() {
  const quotes = [
    ['The fitting was neat and the outfit arrived before my event.', 'Custom client'],
    ['Our school uniform sample was handled professionally.', 'School administrator'],
    ['The training was practical from the first week.', 'Fashion student']
  ];
  ...
  return (
    <section className="bg-white py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="section-title">Trusted by customers and learners</h2>
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {quotes.map(([quote, name]) => (
            <figure key={name} className="rounded-lg border border-ink/10 bg-silk p-6">
              <blockquote className="text-sm leading-6 text-ink/75">"{quote}"</blockquote>
              <figcaption className="mt-4 font-bold text-berry">{name}</figcaption>
            </figure>
          ))}
        </div>
      </div>
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {type === 'custom-order' && <Input name="occasion" label="Occasion" />}
        {type === 'custom-order' && <Input name="measurements" label="Measurements" />}
        {type === 'custom-order' && <Input name="inspirationImages" label="Inspiration image URLs" />}
        ...
        {type === 'uniform-bulk-order' && <Input name="sampleDesigns" label="Sample design URLs" />}
        ...
        {type === 'training-application' && <Input name="duration" label="Preferred duration" />}
...
function Cart({ cart, setCart }) {
  ...
  async function submitOrder(event) {
    ...
    const items = cart.map((item) => ({
      product: item._id.startsWith('f') || item._id.startsWith('demo') ? undefined : item._id,
      name: item.name,
      price: item.price,
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_420px] lg:px-8">
        <div>
          ...
          <div className="mt-7 space-y-3">
            ...
            {cart.map((item) => (
              <div key={item._id} className="flex flex-col gap-3 rounded-lg border border-white/10 bg-white/[0.08] p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="font-bold">{item.name}</h3>
                  <p className="text-sm text-white/65">{money(item.price)} each</p>
                </div>
                <div className="flex items-center gap-3">
                  <button className="icon-button border-white/15 bg-white/10 text-white" onClick={() => updateQuantity(item._id, -1)} aria-label="Decrease quantity">-</button>
                  <span className="w-8 text-center font-bold">{item.quantity}</span>
                  <button className="icon-button border-white/15 bg-white/10 text-white" onClick={() => updateQuantity(item._id, 1)} aria-label="Increase quantity">+</button>
          ...
          <div className="mt-6 space-y-2 border-t border-white/10 pt-5 text-lg font-bold">
            <div className="flex items-center justify-between"><span>Subtotal</span><span>{money(subtotal)}</span></div>
            <div className="flex items-center justify-between text-xl"><span>Total</span><span>{money(total)}</span></div>
          </div>
        </div>
        <form onSubmit={submitOrder} className="rounded-lg bg-white p-5 text-ink">
          <h3 className="mb-4 text-lg font-bold">Checkout summary</h3>
